trigger MKT_ProductQuizCheck on lmscons__Quiz__c (before insert, before update) {
	Set<Id> ProductIdsSet = new Set<Id>();
	Map<Id, List<lmscons__Quiz__c>> ProductIdsListObjectsMap = new Map<Id, List<lmscons__Quiz__c>>();
	Map<Id, List<lmscons__Quiz__c>> ProductIdsListObjectsMapOld = new Map<Id, List<lmscons__Quiz__c>>();
	Map<Id, Id> ProductIdsObjectsIdMapOld = new Map<Id, Id>();
	if (Trigger.isInsert) {

		for (lmscons__Quiz__c NewCourse : trigger.new) {
			if (NewCourse.Product__c != NULL) {
				List<lmscons__Quiz__c> CourseList;
				if (ProductIdsListObjectsMap.containsKey(NewCourse.Product__c)) {
					CourseList = ProductIdsListObjectsMap.get(NewCourse.Product__c);
				}
				else {
					CourseList = new List<lmscons__Quiz__c>();
				}
	...
	if (Trigger.isUpdate) {
		...
		for (lmscons__Quiz__c OldCourse : trigger.old) {
		...
		if (ProductsIdsSet.size() > 0) {
			...
			for(Id ProdId : ProductsIdsList) {
				if(ProductIdsListObjectsMapOld.containsKey(ProdId)) {
					for (lmscons__Quiz__c Coursetem : ProductIdsListObjectsMapOld.get(ProdId)) {
		...
		if (ProductsIdsSet.size() > 0) {
			...
			for(Id ProdId : ProductsIdsList) {
				if(ProductIdsListObjectsMap.containsKey(ProdId)) {
					for (lmscons__Quiz__c Coursetem : ProductIdsListObjectsMap.get(ProdId)) {
